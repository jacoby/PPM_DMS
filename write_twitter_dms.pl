...
my $message = get_next_message( $db ) ;
$twit->update( $message->{ message } ) ;
mark_message_read( $db , $message->{ id } ) ;
...
sub mark_message_read {
    my ( $db , $id ) = @_ ;
    my $sql = <<SQL ;
        update ppm_dms set is_sent = 1 where id = ?
SQL
    my $sth = $db->prepare( $sql ) ;
    $sth->execute( $id ) or croak $db->errstr ;
    my $ptr = $sth->rows() ;
    return $ptr ;
    }
...
sub get_next_message {
    my ( $db ) = @_ ;
    my $sql = <<SQL ;
        select * from ppm_dms where is_sent = 0;
SQL
    my $sth = $db->prepare( $sql ) ;
    $sth->execute() or croak $db->errstr ;
    my $ptr = $sth->fetchall_hashref( 'id' ) ;
    my ( $oldest ) = sort { $a <=> $b } keys %$ptr ;
    return $ptr->{ $oldest } ;
    }
